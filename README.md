...
- Pop audio uses 16 distinct pops cut from five CC0 bubble-wrap recordings, packed into a 92 KB local sound bank with a quiet synthesized pressure layer. Subtle pitch and level variation, irregular 24–42 ms voice spacing, an 85 ms queue limit, and controlled treble keep dense impacts clear. A separate 1.08 MB item bank adds 44 recorded, layered and synthesized cues: rubber mallet thuds, wooden bat cracks, heavy ball impacts, four .45 pistol reports, recorded rocket and cannon blasts, and a soft suction sound made from filtered air noise. Vacuum release fades the existing sound without a separate ending cue. Strong hits use six fuller plastic snaps; finger and suction pops keep the original small bank. Swing audio follows the visible strike; impact speed and position affect gain and panning. Tools leave voice capacity for the plastic pops. The banks load independently with synthesized fallbacks, and pause, mute, reset, and input cancellation stop sustained sounds. See [audio sources](public/audio/CREDITS.md).
...
- `lib/game/tool-synthesis.ts`: cached fallback cues for each item action.
- `lib/game/tool-bank.ts`: frame index for the recorded item atlas.
- `public/audio/`: local pop and item banks with source credits.
- `tools/audio/`: reproducible sound preparation, local pop and item auditions, and source hashes.
