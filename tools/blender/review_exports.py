...
for entry in json.loads((ROOT/'public/models/manifest.json').read_text()):
 ...
 cam.data.ortho_scale=extent*1.48;cam.location=center+Vector((extent*(2.6 if entry['tool']>=6 else 1.7 if slug=='pop-blaster' else .85),-extent*3.5,extent*1.1));aim(cam,center)
 s.render.filepath=str(OUT/f'export-{slug}.png');bpy.ops.render.render(write_still=True)
 if entry['tool']>=6:
  cam.location=center+Vector((extent*2.6,extent*3.5,extent*1.1));aim(cam,center);s.render.filepath=str(OUT/f'export-{slug}-front.png');bpy.ops.render.render(write_still=True)
