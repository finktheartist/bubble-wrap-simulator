...
const server=await createServer({configFile:false,root:resolve(root,'tools/audio'),publicDir:resolve(root,'public'),resolve:{alias:{'@game':resolve(root,'lib/game'),'@before':resolve(output,'before')}},server:{host:'127.0.0.1',port:4180,strictPort:true,fs:{allow:[root]}},plugins:[{name:'save-local-audio-review',configureServer(server){server.middlewares.use('/latest-comparison.mp3',async(_req,res)=>{
