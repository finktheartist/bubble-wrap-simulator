#!/usr/bin/env python3
"""Build the item atlas. Requires Python 3, FFmpeg, and the project's Node/tsx deps."""
...
def suction(kind):
    generator='@/lib/game/suction-synthesis.ts'
    renderer='@/tools/audio/render-suction.ts'
    for path in [generator,renderer]: source_hashes[path]=hashlib.sha256((ROOT/path[2:]).read_bytes()).hexdigest()
    raw=subprocess.check_output(['node','--import','tsx',str(ROOT/renderer[2:]),kind],cwd=ROOT)
    samples=array.array('f',raw)
    return samples,dict(source=generator,renderer=renderer,seed=68127,kind=kind,sampleRate=RATE)
...
# The complete vacuum is original filtered air turbulence, shared with the fallback.
# Preserve the generator's quiet envelope and loop seam instead of renormalizing it.
for kind in ['vacuum-start','vacuum']:
    samples,recipe=suction(kind)
    clips.append(dict(kind=kind,offsetFrames=len(atlas),lengthFrames=len(samples),peakSeconds=0))
    recipes.append(dict(kind=kind,duration=len(samples)/RATE,layers=[dict(**recipe,offset=0)]))
    atlas.extend(round(max(-1,min(1,v))*32767) for v in samples)
...
metadata=dict(url='/audio/item-sounds.wav?v='+digest[:12],sampleRate=RATE,frames=len(atlas),clips=clips)
